...
for i in range(len(item_list)):
    for j in range(len(item_list)):
        ...
        # add 2 seconds to the time bewteen items if the time is not already 0. this creates an extra
...
$t_{ij} = y_{j}$ if item $j$ follows item $i$ in the chosen path, 0 otherwise.
...
(8)&&\sum_{k=2}^{n+1} t_{jk} &= y_j + \sum_{k=2}^{n+1} (d_{jk}+2)x_{jk} &\forall j=1, 2, ..., n \\ \\
